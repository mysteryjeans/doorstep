...
class AdvanceSearchForm(forms.Form):
    ...
    category = forms.ModelChoiceField(required=False, queryset=Category.objects.filter(is_active=True), empty_label='all...')
    manufacturer = forms.ModelChoiceField(required=False, queryset=Manufacturer.objects.filter(is_active=True), empty_label='all...')
    price_from = forms.IntegerField(required=False, label='Price From', widget=forms.TextInput(attrs={'placeholder': 'From...'}))
    price_to = forms.IntegerField(required=False, label='Price To', widget=forms.TextInput(attrs={'placeholder': 'To...'}))
